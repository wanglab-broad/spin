...
def Cal_Spatial_Net(adata, rad_cutoff=None, k_cutoff=None, model='Radius', verbose=True, sample_rate=None):
    """\
    ...
    if model == 'KNN':
        ...
        if sample_rate:
            ...
            indices = np.array([np.random.choice(idxs, size=n_samples, replace=False)
                                for idxs in indices])
            distances = np.array([np.random.choice(dists, size=n_samples, replace=False)
